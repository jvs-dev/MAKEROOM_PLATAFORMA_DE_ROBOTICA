  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <header className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Notificações 🔔</h1>
          <p className="text-slate-500">Fique por dentro das novidades e conquistas.</p>
        </div>
        <div className="flex gap-3">
          {notifications.some(n => !n.read) && (
            <button 
              onClick={markAllAsRead}
              className="px-4 py-2 bg-brand-50 text-brand-600 rounded-xl text-sm font-bold hover:bg-brand-100 transition-colors flex items-center gap-2"
            >
          ...
          {notifications.length > 0 ? (
            notifications.map((notification) => (
              <motion.div
                ...
                className={`bg-white p-6 rounded-3xl shadow-sm border transition-all duration-200 group relative ${
                  notification.read ? 'border-slate-100 opacity-80' : 'border-brand-100 bg-brand-50/10'
                }`}
              >
                {!notification.read && (
                  <div className="absolute left-0 top-6 bottom-6 w-1 bg-brand-500 rounded-r-full" />
                )}
                
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${
                    notification.read ? 'bg-slate-50 text-slate-400' : 'bg-brand-100 text-brand-600'
                  }`}>
                    {notification.title.includes('Temporada') ? <Trophy className="w-6 h-6" /> : <Bell className="w-6 h-6" />}
                  </div>
                  
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className={`font-bold truncate ${notification.read ? 'text-slate-700' : 'text-slate-900'}`}>
                        {notification.title}
                      </h3>
