  ...
  const fetchData = async () => {
    if (auth.currentUser && auth.currentUser.email) {
      try {
        ...
        try {
          const submissionsQuery = query(collection(db, 'submissions'), where('userId', '==', auth.currentUser.uid));
          ...
          >
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                className="flex-1 bg-brand-500 hover:bg-brand-600 disabled:bg-slate-200 text-white font-bold py-3 rounded-2xl transition-all shadow-lg shadow-brand-100 dark:shadow-none flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
