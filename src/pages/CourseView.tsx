...
import { doc, getDoc, collection, query, where, getDocs, addDoc, serverTimestamp, updateDoc, arrayUnion, onSnapshot } from 'firebase/firestore';
...
export default function CourseView() {
  ...
  useEffect(() => {
    ...
    // 1. Fetch Course Data (One-time or Snapshot)
    const unsubscribeCourse = onSnapshot(doc(db, 'courses', id), async (courseDoc) => {
      if (!courseDoc.exists()) {
        navigate('/');
        return;
      }
      ...
      // Fetch lessons and challenges titles (could be made real-time too if needed)
    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', email), (userDoc) => {
      if (userDoc.exists()) {
    ...
    const unsubscribeSubmissions = onSnapshot(submissionsQuery, (snapshot) => {
      const subsList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
      subsList.sort((a, b) => {
  ...
  const allQuizzesPassed = course?.challengeIds.every(id => {
    // Check if any graded submission for this quiz reached the 70% threshold
    return submissions.some(s => s.challengeId === id && s.status === 'graded' && s.grade >= 70);
  });

  const getBestGradeForChallenge = (challengeId: string) => {
    const challengeSubmissions = submissions.filter(s => s.challengeId === challengeId && s.status === 'graded');
    if (challengeSubmissions.length === 0) return 0;
    return Math.max(...challengeSubmissions.map(s => s.grade || 0));
  };
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border border-slate-100 dark:border-white/10 shadow-sm lg:sticky lg:top-8 transition-colors">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-black text-slate-900 dark:text-white">
                  {course.challengeIds.filter(id => {
                    return submissions.some(s => s.challengeId === id && s.status === 'graded' && s.grade >= 70);
                  }).length}/{course.challengeIds.length}
                </span>
              </div>
