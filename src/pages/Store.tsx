...
const ProductCardItem = memo(({ product, setSelectedProduct, cartCount, removeFromCart, addToCart, userRole }: ProductCardProps) => {
  ...
  return (
    <motion.div
      ...
      onClick={(e) => {
        ...
        if (target.closest('.prevent-card-click')) {
          return;
        }
    ...
    >
      {/* Image Section */}
      <div className="aspect-square relative overflow-hidden bg-white dark:bg-white rounded-xl p-3 md:p-6 transition-colors font-sans">
        <img 
      ...
      <div className="p-3 md:p-5 flex-1 flex flex-col justify-between">
        ...
        <div className="mt-2 md:mt-4 flex flex-col items-start sm:flex-row sm:items-center justify-between gap-2 md:gap-3">
          <div className="flex flex-col items-start text-left h-full justify-center">
            {isPromo ? (
              <>
                <span className="text-[10px] md:text-xs text-slate-400 line-through leading-none font-medium">R$ {product.price.toFixed(2)}</span>
          ...
          <div onClick={(e) => e.stopPropagation()} className="prevent-card-click w-full sm:w-auto flex justify-start sm:justify-end shrink-0">
            {cartCount > 0 ? (
              <div className="flex items-center bg-slate-100 dark:bg-white/5 rounded-lg md:rounded-xl p-1 border border-slate-200 dark:border-white/10 w-full sm:w-auto justify-center h-10">
                <button 
                ...
                <span className="text-xs sm:text-sm font-bold w-8 sm:w-6 text-center text-slate-900 dark:text-white">{cartCount}</span>
                <button 
                  onClick={(e) => { e.stopPropagation(); addToCart(product.id); }} 
                  className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-brand-600 transition-colors"
                >
              ...
              <button 
                ...
                className={cn(
                  "w-full sm:w-10 h-10 text-white rounded-lg md:rounded-xl flex items-center justify-center transition-all shadow-md active:scale-95",
                  userRole === 'admin'
                    ? "bg-slate-400 dark:bg-zinc-700 cursor-not-allowed opacity-60"
                    : "bg-brand-500 hover:bg-brand-600"
                )}
              >
                <Plus className="w-5 h-5 sm:block hidden" />
                <span className="sm:hidden text-xs font-black uppercase tracking-widest px-3">Adicionar</span>
              </button>
            )}
          </div>
        </div>
...
export default function Store() {
  ...
  const addToCart = (id: string, e?: React.MouseEvent) => {
    ...
    if (userRole === 'admin') {
      alert('Administradores não podem fazer compras na loja.');
      return false;
    }
  ...
  if (couponDiscountPercent > 0) {
    finalSubtotal = subtotal * (1 - couponDiscountPercent / 100);
  }
  const totalPrice = finalSubtotal + (deliveryMethod === 'delivery' ? deliveryFee : 0);
  ...
  const processCoupon = () => {
    ...
    } else if (cleanCode === 'MAKER20') {
      setCouponCode(cleanCode);
      setCouponDiscountPercent(20);
      setShowCouponModal(false);
      alert('Cupom de 20% aplicado com sucesso!');
    } else {
      setCouponCode('');
      ...
      if (paymentData && paymentData.status === 'approved') {
        ...
        try {
          if (lastOrderId) {
            ...
            const q = query(collection(db, 'orders'), where('paymentId', '==', String(paymentInfo.id)), where('userId', '==', user.uid));
  ...
  return (
    <div className="space-y-12 pb-24 selection:bg-brand-500/30">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar relative z-10 selection:bg-brand-500/30">
                ...
                {totalItems === 0 && (
                  ...
                  {totalItems > 0 && (
                    <div className="p-4 bg-white dark:bg-zinc-900 border-t border-slate-200 dark:border-white/5 space-y-3 relative z-10 shadow-sm mt-4 rounded-xl">
                      ...
                      <div className="flex items-center gap-3 pt-2">
                        <button
                          ...
                          className="flex-shrink-0 w-12 h-12 bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 hover:border-brand-500 hover:bg-brand-50 hover:text-brand-500 shadow-sm flex items-center justify-center transition-all group"
                          title="Aplicar Cupom"
                        >
                          <Ticket className="w-5 h-5 text-slate-400 group-hover:text-brand-500 transition-colors" />
                        </button>
                        <button 
                          disabled={totalItems === 0 || isProcessing || userRole === 'admin'}
                          onClick={handleCheckout}
                          title={userRole === 'admin' ? 'Administradores não podem fazer compras na loja.' : ''}
                          className="flex-1 h-12 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:bg-slate-400 text-white font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95 text-[10px] uppercase tracking-widest disabled:cursor-not-allowed"
                        >
                          {isProcessing ? (
        ...
        {showCouponModal && (
          <div className="fixed inset-0 z-[650] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl bg-white dark:bg-zinc-900 relative z-10 p-6 md:p-8"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button 
                  onClick={() => setShowCouponModal(false)}
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') processCoupon();
                    }}
                    placeholder="Ex: MAKER10"
                    className="w-full bg-slate-50 dark:bg-zinc-800 border-2 border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none focus:border-brand-500 dark:focus:border-brand-500 font-bold placeholder:font-normal uppercase"
                    autoFocus
                  />
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:bg-slate-400 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg text-sm uppercase tracking-wider"
                >
                  Validar
                </button>
              </div>
        ...
        {showCheckoutForm && (
          ...
          >
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="w-full h-full overflow-y-auto overflow-x-hidden custom-scrollbar shadow-2xl relative z-10 bg-white dark:bg-zinc-900"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-6 md:p-10 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/5 backdrop-blur-md">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="flex items-center justify-center">
                    <Package className="w-8 h-8 md:w-12 md:h-12 text-brand-500" />
                  </div>
                  <div>
              ...
              <div className="p-6 md:p-10 space-y-8 md:space-y-10 selection:bg-brand-500/30 max-w-3xl mx-auto w-full">
                <div className="space-y-6 md:space-y-8">
                  <div className="grid grid-cols-1 gap-6 md:gap-8">
                    <div>
                      <label className="block text-xs md:text-sm font-mono font-black text-brand-500 uppercase tracking-[0.3em] mb-2 md:mb-3">NOME COMPLETO</label>
                      <input 
                        type="text"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
                      <div>
                        <label className="block text-xs md:text-sm font-mono font-black text-brand-500 uppercase tracking-[0.3em] mb-2 md:mb-3">DOCUMENTO CPF</label>
                        <input 
                          type="text"
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-mono font-black text-brand-500 uppercase tracking-[0.3em] mb-2 md:mb-3">CANAL DE CONTATO</label>
                        <input 
                          type="text"
                    ...
                    <div className="space-y-3 md:space-y-4">
                      <label className="block text-xs md:text-sm font-mono font-black text-brand-500 uppercase tracking-[0.3em]">MÉTODO DE LOGÍSTICA</label>
                    ...
                    {deliveryMethod === 'pickup' && (
                      ...
                      >
                        <label className="block text-xs md:text-sm font-mono font-black text-brand-500 uppercase tracking-[0.3em]">SELECIONE A ESCOLA</label>
                        <select 
                    ...
                    {deliveryMethod === 'delivery' && (
                      ...
                      >
                         <div className="space-y-4">
                          <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em]">GEOPOSICIONAMENTO</label>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                           <div>
                              <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em] mb-2">CEP</label>
                              <input 
                                type="text"
                           ...
                           <div>
                              <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em] mb-2">CIDADE</label>
                              <input 
                                type="text"
                        ...
                        <div className="space-y-6">
                           <div>
                              <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em] mb-2">RUA E NÚMERO</label>
                              <input 
                                type="text"
                           ...
                           <div className="grid grid-cols-2 gap-6">
                            <div>
                                <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em] mb-2">BAIRRO</label>
                                <input 
                                  type="text"
                            ...
                            <div>
                                <label className="block text-xs md:text-sm font-mono font-black text-brand-600 dark:text-brand-500 uppercase tracking-[0.3em] mb-2">REFERÊNCIA</label>
                                <input 
                                  type="text"
                ...
                <div className="mt-8 pt-8 border-t border-slate-200 dark:border-white/10 flex items-center gap-4 max-w-3xl mx-auto w-full">
                  <button
                    onClick={handleApplyCoupon}
                    className="flex-shrink-0 w-14 h-14 md:w-16 md:h-16 bg-white dark:bg-white/5 rounded-xl md:rounded-2xl border border-slate-200 dark:border-white/10 hover:border-brand-500 hover:bg-brand-50 hover:text-brand-500 shadow-sm flex items-center justify-center transition-all group"
                    title="Aplicar Cupom"
                  >
                    <Ticket className="w-6 h-6 text-slate-400 group-hover:text-brand-500 transition-colors" />
                  </button>
                  <button 
                    ...
                    onClick={handleCheckout}
                    className="flex-1 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 disabled:opacity-20 disabled:grayscale text-white font-black uppercase tracking-[0.1em] md:tracking-[0.2em] h-14 md:h-16 rounded-xl md:rounded-2xl transition-all shadow-xl flex items-center justify-center gap-2 md:gap-3 group active:scale-95 text-xs md:text-sm"
                  >
                    PAGAMENTO
      ...
      <AnimatePresence>
        {showOrders && (
          <div className="fixed inset-0 z-[400] flex bg-slate-900/60 backdrop-blur-md">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-zinc-900 w-full h-full shadow-2xl overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col pt-8 md:pt-0"
            >
              <div className="p-5 md:p-8 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/5 shrink-0">
              ...
              <div className="flex-1 p-4 md:p-8 space-y-4 md:space-y-6 selection:bg-brand-500/30 text-sm">
                {userOrders.length === 0 ? (
                  ...
                  <div className="w-full max-w-3xl mx-auto flex flex-col gap-6 pb-20">
                    {userOrders.map((order) => (
                      <div key={order.id} className="group p-5 md:p-6 bg-white dark:bg-zinc-800 rounded-3xl border border-slate-100 dark:border-white/10 hover:border-brand-200 dark:hover:border-brand-500/50 hover:shadow-xl hover:shadow-brand-50/50 dark:hover:shadow-none transition-all space-y-6 relative overflow-hidden flex flex-col">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-slate-50 dark:bg-zinc-700/50 rounded-2xl flex items-center justify-center border border-slate-100 dark:border-white/5 shrink-0">
                              <Package className="w-5 h-5 text-slate-400 dark:text-slate-500" />
                            </div>
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Pedido #{order.id.slice(-8).toUpperCase()}</p>
                                <span className={cn(
                                  "px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest text-center shadow-sm dark:shadow-none min-w-max",
                                  ...
                                  order.status === 'Entregue' ? "bg-slate-100 dark:bg-zinc-700 text-slate-600 dark:text-slate-400 shadow-slate-100" :
                                  order.status === 'Pago' ? "bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-500/10 dark:border-emerald-500/20" :
                                  "bg-slate-50 dark:bg-zinc-700 text-slate-400 dark:text-slate-400"
                                )}>
                                  {order.status}
                                </span>
                              </div>
                          ...
                          <div className="text-left sm:text-right">
                            <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Valor Total</p>
                            <p className="text-lg font-black text-slate-900 dark:text-white">R$ {order.total.toFixed(2)}</p>
                          </div>
                        </div>

                        {/* Delivery Info */}
                        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-zinc-700/30 p-3 rounded-xl border border-slate-100 dark:border-white/5 relative z-10">
                          {order.deliveryMethod === 'pickup' ? (
                            <>
                              <Home className="w-4 h-4 text-brand-500 shrink-0" />
                              <span className="text-sm font-bold line-clamp-1">Retirada: {order.schoolName || 'Escola'}</span>
                            </>
                          ) : (
                            <>
                              <Navigation className="w-4 h-4 text-brand-500 shrink-0" />
                              <span className="text-sm font-bold line-clamp-1">Entrega em Casa</span>
                            </>
                          )}
                        </div>
                        ...
                        <div className="relative z-10 flex-1">
                          <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Itens Solicitados ({order.productIds?.length || 0})</p>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[160px] overflow-y-auto custom-scrollbar pr-2">
                            {order.productIds?.map((pid: string) => {
                              ...
                              return (
                                <div key={pid} className="flex items-center gap-3 bg-white dark:bg-zinc-800 p-2 rounded-xl border border-slate-100 dark:border-white/5 shadow-sm">
                                  {p ? (
                                    <>
                                      <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-zinc-700 p-1 flex-shrink-0 flex items-center justify-center">
                                        <img src={p.imageUrl} alt={p.name} className="w-full h-full object-contain" />
                                      </div>
                                      <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 line-clamp-1">{p.name}</span>
                                    </>
                                  ) : (
                                    <span className="text-xs font-bold text-slate-400 px-2 py-1">Produto Indisponível</span>
                                  )}
                                </div>
                              );
                            })}
                        ...
                        {/* Actions */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-4 border-t border-slate-50 dark:border-white/5 relative z-10 mt-auto">
                          <button 
                            onClick={() => generateReceipt(order)}
                            className="bg-brand-500 text-white font-bold py-2.5 px-3 rounded-xl hover:bg-brand-600 transition-all flex items-center justify-center gap-2 text-[11px] shadow-sm shadow-brand-100 dark:shadow-none min-w-0"
                          >
                          ...
                          {order.status === 'Aguardando Pagamento' && (
                            <button 
                              ...
                              className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-2.5 px-3 rounded-xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all flex items-center justify-center gap-2 text-[11px] shadow-sm min-w-0"
                            >
                          ...
                          <button className="bg-slate-100 dark:bg-zinc-700 text-slate-600 dark:text-slate-300 font-bold py-2.5 px-3 rounded-xl hover:bg-slate-200 dark:hover:bg-zinc-600 transition-all flex items-center justify-center gap-2 text-[11px] min-w-0">
                          ...
                          {order.status === 'Aguardando Pagamento' && (
                            <button 
                              ...
                              className="bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 font-bold py-2.5 px-3 rounded-xl hover:bg-red-100 dark:hover:bg-red-500/20 transition-all flex items-center justify-center gap-2 text-[11px] min-w-0"
                            >
                          ...
                          {order.status !== 'Entregue' && order.status !== 'Cancelado' && order.status !== 'Aguardando Pagamento' && (
                            <button 
                              ...
                              className="bg-emerald-500 text-white font-bold py-2.5 px-3 rounded-xl hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 text-[11px] shadow-sm shadow-emerald-100 dark:shadow-none min-w-0"
                            >
      ...
      {paymentStatus && (
        <div 
          className="fixed inset-0 z-[700] bg-slate-900/80 backdrop-blur-md flex"
        >
          <div 
            className="bg-white dark:bg-zinc-900 w-full h-full relative overflow-y-auto overflow-x-hidden custom-scrollbar"
          >
            <div className="p-6 text-center selection:bg-brand-500/30">
                ...
                <>
                  {paymentMethod === 'pix' && paymentInfo ? (
                    <>
                      ...
                      <div className="w-full max-w-sm mx-auto bg-slate-50 dark:bg-white/5 p-4 rounded-2xl mb-6 border border-slate-100 dark:border-white/5">
                        <img 
                      ...
                      <div className="space-y-3">
                        <button
                          ...
                          className="w-full max-w-sm mx-auto py-3 bg-brand-500 text-white rounded-xl font-bold text-sm hover:bg-brand-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-500/20"
                        >
                    ...
                    <div className="py-4">
                      ...
                      <button 
                        ...
                        className="w-full max-w-sm mx-auto bg-slate-900 dark:bg-brand-500 text-white font-bold py-3.5 rounded-xl hover:bg-slate-800 dark:hover:bg-brand-600 transition-all text-sm shadow-lg"
                      >
              ...
              {paymentStatus === 'rejected' && (
                <div className="py-4">
                  ...
                  <button 
                    ...
                    className="w-full max-w-sm mx-auto bg-slate-900 dark:bg-zinc-800 text-white font-bold py-3.5 rounded-xl hover:bg-slate-800 dark:hover:bg-zinc-700 transition-all text-sm shadow-lg"
                  >
                    Tentar Novamente
                  </button>
                </div>
