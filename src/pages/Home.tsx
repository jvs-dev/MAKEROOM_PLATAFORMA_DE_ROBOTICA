...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', email), async (userDoc) => {
      if (userDoc.exists()) {
        const userDataObj = userDoc.data();
        // Ensure main admin always has admin privileges in the UI and DB
        if (email === 'jvssilv4@gmail.com' && (!userDataObj.admin || userDataObj.role !== 'admin')) {
          await updateDoc(doc(db, 'users', email), { admin: true, role: 'admin' });
        }
        setUserData(userDataObj);
        setUserTeamId(userDataObj.teamId);
        ...
        const isMainAdmin = email === 'jvssilv4@gmail.com';
        const newUser = { 
          uid: uid,
    ...
    const unsubscribeCourses = onSnapshot(coursesQuery, (snapshot) => {
      const coursesList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
      setCourses(coursesList);
    }, (err) => {
      handleFirestoreError(err, OperationType.LIST, 'courses');
    });

    // 4. Listen to Submissions
    const submissionsQuery = query(
      collection(db, 'submissions'),
      where('userId', '==', uid)
    );
    const unsubscribeSubmissions = onSnapshot(submissionsQuery, (snapshot) => {
      // Sort in memory to avoid needing composite index for query
      const subsList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      subsList.sort((a: any, b: any) => {
    ...
    // 5. Fetch Announcements (one-time is fine for banners, or listen if you prefer)
    getDocs(query(
  ...
  const getCourseProgress = (course: any) => {
    ...
    const totalItems = (course.lessonIds?.length || 0) + (course.challengeIds?.length || 0);
    if (totalItems === 0) return 100;
    
    const completedLessonsCount = (course.lessonIds || []).filter((id: string) => 
      userData.completedLessons?.includes(id)
    ).length;
    
    const completedQuizzesCount = (course.challengeIds || []).filter((cid: string) => {
      // Find all graded submissions for this challenge and check if any is above 70%
      return submissions.some(s => s.challengeId === cid && s.status === 'graded' && s.grade >= 70);
    }).length;
    
    return Math.round(((completedLessonsCount + completedQuizzesCount) / totalItems) * 100);
  };
  ...
  const lastCourse = (() => {
    ...
    if (submissions && submissions.length > 0) {
      // Submissions are already sorted by date in onSnapshot
    ...
    // 2. Try to find the course with the most recent lesson completion
    if (userData.completedLessons && userData.completedLessons.length > 0) {
      const lastLid = userData.completedLessons[userData.completedLessons.length - 1];
      const course = courses.find(c => c.lessonIds?.includes(lastLid));
      if (course && !isCourseCompleted(course)) {
        return course;
      }
    }

    // 3. Fallback: Find the course with progress > 0 and < 100
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      {lastCourse && (
        ...
        >
          <div className="relative bg-white dark:bg-zinc-900/40 p-4 md:p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 flex flex-col md:flex-row items-center gap-6 md:gap-8 shadow-sm hover:shadow-md hover:border-brand-500/30 transition-all">
            <div className="w-24 h-24 md:w-40 md:h-40 bg-slate-50 dark:bg-white/5 rounded-2xl md:rounded-[2rem] border border-slate-100 dark:border-white/5 shrink-0 overflow-hidden relative shadow-inner">
              <img 
                src={lastCourse.thumbnail || 'https://makeroom2.vercel.app/lampLogo.svg'} 
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {availableCourses.map((course) => (
            ...
            >
              <Link to={`/courses/${course.id}`} className="flex flex-col h-full bg-white dark:bg-zinc-800/80 rounded-[2rem] border border-slate-100 dark:border-white/5 overflow-hidden transition-all hover:border-brand-300 dark:hover:border-brand-500/30 hover:shadow-2xl hover:shadow-brand-500/10 dark:hover:shadow-brand-500/5">
                <div className="aspect-[4/3] relative overflow-hidden bg-slate-50 dark:bg-zinc-900/50">
                  <img 
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  ...
                  <div className="absolute bottom-4 left-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/20 backdrop-blur-md border border-white/20 shadow-sm text-white">
                    <BookOpen size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">{course.lessonIds?.length || 0} Módulos</span>
                  </div>
                </div>

                <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-lg md:text-xl font-bold tracking-tight text-slate-900 dark:text-white mb-3 line-clamp-2">{course.title}</h3>
                    <p className="text-slate-500 dark:text-slate-400 text-sm line-clamp-2 leading-relaxed mb-6">{course.description}</p>
                  </div>

                  <div className="space-y-4">
                    <div className="flex justify-between items-end text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                      <span>Progresso</span>
                          ...
                          <div key={comment.id} className="glass p-4 rounded-2xl border border-white/5 group hover:border-brand-500/30 transition-all">
                            <div className="flex gap-4">
                              ...
                              <div className="flex-1 space-y-2">
                                <div className="flex items-center justify-between">
                                  <span className="text-xs font-black uppercase text-brand-300 tracking-tight">
                                    {comment.userName.split(' ').slice(0, 2).join(' ')}
                                  </span>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-4 flex items-center justify-center gap-3">
                <Award className="text-brand-500" size={36} />
                Novo Certificado!
              </h2>
