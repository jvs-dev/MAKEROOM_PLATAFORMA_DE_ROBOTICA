  ...
  return (
    <div className="space-y-8">
      <header className="flex items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Gerenciar Desafios ⚡</h1>
          <p className="text-slate-500">Crie quizzes e atividades para os alunos.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest">Desafio</th>
              <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest">Tipo</th>
              <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest">Pontos</th>
              <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">Ações</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {challenges.map((challenge) => (
              <tr key={challenge.id} className="hover:bg-slate-50 transition-colors group">
                <td className="p-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      challenge.type === 'quiz' ? 'bg-blue-50' : 'bg-purple-50'
                    }`}>
                      {challenge.type === 'quiz' ? (
                ...
                <td className="p-6 font-bold text-brand-600">
        ...
        {challenges.length === 0 && !isLoading && (
          <div className="p-12 text-center text-slate-400 italic">Nenhum desafio cadastrado.</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full">
            <div className="flex items-center gap-4 text-red-500 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">Excluir Desafio?</h2>
                <p className="text-sm text-slate-500">Esta ação não pode ser desfeita.</p>
              </div>
            </div>
            
            <p className="text-slate-600 mb-8">
              Você está prestes a excluir o desafio <span className="font-bold text-slate-900">"{deleteConfirmation.title}"</span>. 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-4">
                ...
                {!editingChallenge && formData.type === 'quiz' && (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-brand-50 text-brand-600 rounded-xl font-bold text-sm hover:bg-brand-100 transition-colors"
                  >
            ...
            {showAiInput && (
              <div className="mb-8 p-6 bg-brand-50 rounded-3xl border border-brand-100 space-y-4">
                ...
                <div className="flex gap-3">
                  <input 
                    ...
                    className="flex-1 p-3 bg-white border border-brand-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none"
                  ...
                  <button
                    ...
                    className="bg-brand-500 hover:bg-brand-600 text-white font-bold px-6 rounded-xl transition-all disabled:opacity-50 flex items-center gap-2"
                  >
                    {isGenerating ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Pontuação</label>
              ...
              {formData.type === 'quiz' && (
                <div className="space-y-6 pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-slate-900">Perguntas do Quiz</h3>
                    <button 
                      type="button"
                      onClick={addQuestion}
                  ...
                  <div className="space-y-8">
                    {formData.questions.map((q, qIndex) => (
                      <div key={q.id} className="p-6 bg-slate-50 rounded-2xl space-y-4 relative group/q">
                        <button 
                          type="button"
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Pergunta {qIndex + 1}</label>
                          <input 
                            required
                            value={q.text}
                          ...
                          {(['A', 'B', 'C', 'D'] as const).map((opt) => (
                            <div key={opt} className="space-y-1">
                              <div className="flex items-center justify-between px-1">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Opção {opt}</label>
                                <button 
                                  type="button"
