  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Gerenciar Cursos 🎓</h1>
          <p className="text-slate-500">Crie trilhas de aprendizado com certificados.</p>
        </div>
        <button
          onClick={() => { resetForm(); setIsModalOpen(true); }}
          className="bg-brand-500 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-brand-600 transition-all shadow-lg shadow-brand-100"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 bg-white rounded-2xl border border-slate-100 shadow-sm focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none transition-all"
        />
      </div>
          ...
          {filteredCourses.map(course => (
            <div key={course.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden group">
              <div className="aspect-video bg-slate-100 relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-brand-600 flex items-center gap-1 shadow-sm">
      ...
      {deleteConfirmation && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-[2rem] shadow-2xl max-w-md w-full">
            <div className="flex items-center gap-4 text-red-500 mb-6">
              <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center">
                <Trash2 className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-xl font-bold text-slate-900">Excluir Curso?</h2>
                <p className="text-sm text-slate-500">Esta ação não pode ser desfeita.</p>
              </div>
            </div>
            
            <p className="text-slate-600 mb-8">
              Você está prestes a excluir o curso <span className="font-bold text-slate-900">"{deleteConfirmation.title}"</span>.
            </p>
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-red-100 flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {isDeleting ? (
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Título do Curso</label>
                    <input
                      type="text"
                      value={title}
                      onChange={(e) => setTitle(e.target.value)}
                      className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 outline-none"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 outline-none resize-none"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">Pontos de Recompensa</label>
                      <input
                        type="number"
                        value={pointsReward}
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">URL da Thumbnail</label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 outline-none"
                          placeholder="https://..."
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Selecionar Aulas ({selectedLessons.length})</label>
                    <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 max-h-[200px] overflow-y-auto space-y-2">
                      {lessons.map(lesson => (
                        <button
                          key={lesson.id}
                          type="button"
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Selecionar Quizes ({selectedQuizzes.length})</label>
                    <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 max-h-[200px] overflow-y-auto space-y-2">
                      {quizzes.map(quiz => (
                        <button
                          key={quiz.id}
              ...
              <div className="flex items-center justify-end gap-4 pt-6 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="bg-brand-500 text-white px-8 py-3 rounded-xl font-bold hover:bg-brand-600 transition-all shadow-lg shadow-brand-100"
                >
